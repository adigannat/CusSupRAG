...
- Scope (initial): Top services by demand (EC2, S3, IAM, Lambda, API Gateway, DynamoDB, RDS/Aurora, ECS/EKS, CloudWatch/CloudTrail, VPC/Networking, CloudFormation/Terraform interoperability).
...
- Embedding model: Pick high-recall model available in stack (e.g., OpenAI text-embedding-3-large or equivalent OSS) and validate with a pilot eval.
- Store: Vector store with metadata filters (pgvector/Weaviate/Elasticsearch) + durable object storage for raw markdown + manifest of source/version.
...
- M1: Skeleton pipeline (fetch -> clean -> chunk -> embed -> store) for 2 services (S3, IAM) + basic hybrid search + eval harness.
- M2: Expand to core list; add reranker + citation-enforced generation; set up scheduler and metrics.
- M3: Quality hardening (eval expansion, dedup tuning, chunking tweaks), observability, and alerts.
- M4: Optional: live AWS tool and UI polish (source-rich answers, examples-first layout).
